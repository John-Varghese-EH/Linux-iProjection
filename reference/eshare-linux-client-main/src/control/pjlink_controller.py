...
---------------------------
  ...
  2. Receive greeting: "%1PJLINK 0\\r" (0 = no auth) or
                       "%1PJLINK 1 <salt>\\r" (1 = MD5 auth required)
  3. If auth: prepend MD5(salt + password) to every command
  4. Commands: "%1<CMD> ?\\r"   → query
               "%1<CMD> <VAL>\\r" → set
  5. Response: "%1<CMD>=<RESP>\\r"

Class 2 extensions (CLSS 2, SRCH, AVMT, etc.) follow the same framing
with "%2" prefix.
...
-----
    import asyncio
    from src.control.pjlink_controller import PJLinkController
